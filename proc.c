...
PUBLIC void PopQueue(ReadyQueue *Q) 
{ 
    if(EmptyQueue(Q))  
    {  
 ...
 /*将进程插入到就绪队列尾部*/
PUBLIC void InsertLast(struct proc *p,ReadyQueue *Q)   
{
    if(FullQueue(Q))  
...
/*******Last Queue*****************************/
PUBLIC void RoundRun(ReadyQueue *queue)    /*时间片轮转调度算法*/  
{         
  
//printf("~round~"); 
            if(p_proc_ready->ticks == 0) /*进程执行完毕*/  
            {   
                PopQueue(queue);  
            }  
            ...
            GetFirst(queue); 
            while(IsBlock) //this queue's pros all be block
            {
                GetFirst(queue);
            } 
...
/*PUBLIC void Insert_all()
{
        ...
        {
             InsertLast(proc_table[j],&Queue[0]);
        }
        //init_new();
        IsEmpty = 0; //no empty    
}*/
 void Insert_new()
{
        ...
        for(j = 0; j < NR_TASKS+NR_PROCS; j++)
        {
             if (proc_table[j].p_flags == 0)
             {
                 proc_table[j].ticks = proc_table[j].priority;
                 InsertLast(&proc_table[j],&Queue[0]);
             }
        }
        IsEmpty = 0; //no empty
}
PUBLIC void initPro_ready()
{
    IsBlock = 0;
    ReadyQueue *queue;  
    ...
    p_queue_ready = queue;      
    GetFirst(p_queue_ready);
}

PUBLIC void MultiLevel()   /*多级调度算法，每次执行一个时间片*/  
{  
        if(p_queue_ready == &Queue[2])  //last queue
        {  
            RoundRun(p_queue_ready);  
            return;  
        }                
        if(p_proc_ready->ticks == 0) //进程执行完毕 
        {  
                PopQueue(p_queue_ready);  
                //printf("this process end!");  
        }  
        else if(p_proc_ready->count == p_queue_ready->round)//时间片用完  
        {  
                PopQueue(p_queue_ready);
                p_proc_ready->count = 0;   //计数器清零，为下次做准备  
                InsertLast(p_proc_ready,p_queue_ready+1);
              //  printf("Time up, jump to next queue!");                  
        }  
        if(EmptyQueue(p_queue_ready))//if is empty,就绪队列指针下移 
        { 
            p_queue_ready++;
        }        
        GetFirst(p_queue_ready);  
...
PUBLIC void schedule()
{
	...
	int		greatest_ticks = 0;
		...
		if (!greatest_ticks)
			for (p = &FIRST_PROC; p < proc_table+NR_TASKS+2 /*p <= &LAST_PROC*//*; p++)
				if (p->p_flags == 0)
					p->ticks = p->priority;
	}*/
}
